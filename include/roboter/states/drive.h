 ...
 * @note Activates backward motor movement after SETTINGBACKWAD_DELAY_TRESHOLD delay
 */
...
/**
 * @brief round end state entry function
 * @param inst State instance (unused)
 * @note Sets LF_LMR on shift register, prints round completion messages
 */
void round_end_on_entry(t_state *inst __attribute__((unused)));

/**
 * @brief round_end state update function
 * @param inst State instance (unused)
 * @note Currently no update behavior implemented
 */
void round_end_on_update(t_state *inst __attribute__((unused)));

/**
 * @brief Stop state entry function
 * @param inst State instance (unused)
 * @note Stops all motors
 */
