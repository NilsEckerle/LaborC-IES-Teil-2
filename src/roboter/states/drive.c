...
void drive_logic_super_state_on_update(t_state *inst __attribute__((unused))) {
  ...
  if (CLOCK_get_milliseconds() - ui32_DRIVE_MSG_time_last_ms > DRIVING_MSG_PERIOD_MS) {
    UI(MSG_FMT_DRIVING_ROUND, tp_robi->i8_current_round);
...
void drive_through_start_on_entry(t_state *inst __attribute__((unused))) {
  ...
  tp_robi->ui32_last_lap_time_ms = CLOCK_get_milliseconds();
...
void backwards_on_update(t_state *inst __attribute__((unused))) {
  if (CLOCK_get_milliseconds() - *inst->ui32p_state_entry_time_ms > SETTINGBACKWAD_DELAY_TRESHOLD) {

    ENGINE_set_duty_cicle(ENGINE_PWM_LEFT, SETTING_ENGINE_PWM_POWER_BACKWARD);
    ENGINE_set_duty_cicle(ENGINE_PWM_RIGHT, CALCULATE_PERCENT(SETTING_ENGINE_PWM_POWER_BACKWARD, 90));
...
void round_end_on_entry(t_state *inst __attribute__((unused))) {
  t_roboter *tp_robi = ROBOTER_get_instance();
  ...
  // print last round time
  if (tp_robi->b_round_timing) {
    uint32_t ui32_current_time_ms = CLOCK_get_milliseconds();
    uint32_t ui32_current_lap_ms = ui32_current_time_ms - tp_robi->ui32_last_lap_time_ms;
    UI(MSG_FMT_ROUND_TIMING, tp_robi->i8_current_round, ui32_current_lap_ms/1000, ui32_current_lap_ms%1000);
    tp_robi->ui32_last_lap_time_ms = ui32_current_time_ms;
  }

  return;
}

void round_end_on_update(t_state *inst __attribute__((unused))) { return; }
...
void pause_on_update(t_state *inst __attribute__((unused))) {
  ...
  if (CLOCK_get_milliseconds() - ui32_PAUSE_SHIFT_time_last_ms > SETTING_PAUSE_SHIFT_PERIOD_MS) {
